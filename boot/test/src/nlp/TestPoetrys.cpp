...
[[maybe_unused]] static void testMatchDataset() {
    ...
    lifuren::files::listFiles(files, lifuren::config::CONFIG.mark.begin()->path, { ".json" });
    int64_t fSize    = 0LL;
    int64_t count    = 0LL;
    int64_t total    = 0LL;
    int64_t ciCount  = 0LL;
    ...
    for(const auto& file : files) {
        ++fSize;
        std::string&& json = lifuren::files::loadFile(file);
        auto&& poetries = nlohmann::json::parse(json);
        for(const auto& poetry : poetries) {
            lifuren::poetrys::Poetry value = poetry;
