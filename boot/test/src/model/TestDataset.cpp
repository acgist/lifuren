...
[[maybe_unused]] static void testRawDataset() {
    ...
    lifuren::dataset::RawDataset dataset(
        210LL,
        5,
        features,
        1,
...
[[maybe_unused]] static void testFillRhythm() {
    ...
    lifuren::dataset::poetry::fillRhythm(128, vector, &rhythm);
...
[[maybe_unused]] static void testFileDataset() {
    lifuren::dataset::FileDataset dataa(
        5,
    ...
    SPDLOG_DEBUG("当前数量：{}", size);
    lifuren::dataset::FileDataset datab(
        5,
        lifuren::file::join({lifuren::config::CONFIG.tmp, "sex"}).string(),
        { ".jpg" },
        [](const std::string& path, std::vector<std::vector<float>>& features) {
            // SPDLOG_DEBUG("读取文件：{}", path);
            features.push_back(std::vector<float>{1.0F, 2.0F, 3.0F, 4.0F, 5.0F});
...
[[maybe_unused]] static void testShardingDataset() {
    ...
    auto dataset = std::make_shared<lifuren::dataset::RawDataset>(
        210LL,
        5,
        features,
        1,
    ...
    auto [traina, vala, testa] = lifuren::dataset::ShardingDataset::make(dataset);
    ...
    vala.batchGet(0, f, l);
    auto [trainb, valb, testb] = lifuren::dataset::ShardingDataset::make(dataset, 0, 2, 3, 1);
...
[[maybe_unused]] static void testLoadImageFileDataset() {
    ...
    auto data_loader = lifuren::dataset::loadImageFileDataset(200, 200, 5, lifuren::file::join({lifuren::config::CONFIG.tmp, "sex"}).string(), ".jpg", mapping);
    ...
    lifuren::image::write(lifuren::file::join({lifuren::config::CONFIG.tmp, "loader.png"}).string(), data, 200, 200);
