...
[[maybe_unused]] static void testFunction() {
    ...
    ggml_tensor * input  = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 2, 3);
    ...
    ggml_tensor * x      = lifuren::function::permute(ctx, input, 1, 0);
    ggml_tensor * output = lifuren::function::flatten(ctx, x);
    ggml_cgraph * gf     = ggml_new_graph_custom(ctx, 1024, true);
    ggml_build_forward_expand(gf, output);
    ggml_graph_compute_with_ctx(ctx, gf, 4);
    std::ofstream out;
