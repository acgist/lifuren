...
// 消息列表
static std::list<std::tuple<Fl_Text_Buffer*, Fl_Text_Display*>> messageList{};
// 配置窗口
...
static Fl_Text_Buffer* appendMessage(const std::string& message);
...
lifuren::ChatWindow::ChatWindow(int width, int height, const char* title) : ModelWindow(width, height, title) {
    ...
    this->clientPtr = new RestChatClient{options};
}
...
void lifuren::ChatWindow::drawElement() {
    ...
    // 回车发送
    messageEditorPtr->when(FL_Enter);
...
static void appendMessage(const std::string& message, bool done) {
    static Fl_Text_Buffer* bufferCachePtr{nullptr};
    if(bufferCachePtr != nullptr) {
        bufferCachePtr->append(message.c_str());
        if(done) {
            bufferCachePtr = nullptr;
        }
        return;
    }
    Fl_Text_Buffer* bufferPtr = appendMessage(message);
    if(!done) {
        bufferCachePtr = bufferPtr;
    }
}
...
static Fl_Text_Buffer* appendMessage(const std::string& message) {
    ...
    messageList.push_back(std::make_tuple(bufferPtr, displayPtr));
    return bufferPtr;
}
