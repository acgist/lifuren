...
[[maybe_unused]] static void testNoise() {
    ...
    auto beta = torch::sqrt(1.0 - torch::pow(alpha, 2));
    auto bar_alpha = torch::cumprod(alpha, 0);
    auto bar_beta = torch::sqrt(1.0 - bar_alpha.pow(2));
    auto sigma = beta.clone();
    ...
    tensor = torch::concat({ tensor, tensor, tensor, tensor, tensor, tensor }, 0);

    const auto& batch_images = tensor;
    auto batch_size = batch_images.size(0);
    std::vector<int> steps{0, 10, 50, 100, 200, 999};
    steps.resize(batch_size);
    auto batch_steps = torch::tensor(steps, torch::TensorOptions().dtype(torch::kLong));
    ...
    std::cout << bar_alpha_ << std::endl;
    std::cout << batch_steps << std::endl;
    std::cout << batch_bar_alpha << std::endl;
    std::cout << batch_bar_beta << std::endl;
    std::cout << bar_alpha_.sizes() << std::endl;
    std::cout << batch_noise.sizes() << std::endl;
    std::cout << batch_steps.sizes() << std::endl;
    std::cout << batch_bar_alpha.sizes() << std::endl;
    std::cout << batch_bar_beta.sizes() << std::endl;
    std::cout << batch_noise_images.sizes() << std::endl;
    ...
    lifuren::dataset::image::tensor_to_mat(output_noise, batch_noise       .to(torch::kFloat32).to(torch::kCPU));
