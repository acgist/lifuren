...
public:
    UpsampleImpl(const int channels, const int num_groups = 16, const int ratio_kernel_size = 2) {
...
public:
    AttentionBlockImpl(const int channels, const int num_heads, const int embedding_dims, const int num_groups = 16, const float dropout = 0.3) {
        SPDLOG_INFO(
...
public:
    ResidualBlockImpl(const int in, const int out, const int embedding_dims, const int num_groups = 16) {
        SPDLOG_INFO(
            "residual block in = {} out = {} embedding_dims = {} num_groups = {}",
            in, out, embedding_dims, num_groups
        );
        if(in == out) {
...
public:
    TimeEmbeddingImpl(
        const int width, const int height, const int in, const int out,
        const int scale = 4, const int channels = 32,
        const int num_heads = 8, const int num_groups = 16
    ) {
        SPDLOG_INFO(
            "time embedding width = {} height = {} in = {} out = {} scale = {} channels = {} num_heads = {} num_groups = {}",
            width, height, in, out, scale, channels, num_heads, num_groups
        );
...
public:
    UNetImpl(
        ...
        const int out_channels = 3, const int base_channels = 16,
        const int num_res = 2, const int num_heads = 8, const int num_groups = 16,
        const int min_down_pixel = 4, const int max_attn_pixel = 32,
        const std::vector<int>& scales = { 2, 2, 4, 4, 8 }
    ) {
        SPDLOG_INFO(
            "unet width = {} height = {} channels = {} embedding_dims = {} out_channels = {} base_channels = {} num_res = {} num_groups = {} num_heads = {} min_down_pixel = {} max_attn_pixel = {}",
            width, height, channels, embedding_dims, out_channels, base_channels, num_res, num_groups, num_heads, min_down_pixel, max_attn_pixel
        );
        int index = 0;
        ...
        for (size_t i = 0; i < scales.size(); ++i) {
            ...
            for (int j = 0; j < num_res; ++j) {
                encoder_channels.emplace_back(current_channels, scale * base_channels);
                encoder_blocks.insert(
                    "res_" + std::to_string(i) + "_" + std::to_string(j),
                    lifuren::nn::ResidualBlock(current_channels, scale * base_channels, embedding_dims, num_groups).ptr()
                );
                current_channels = scale * base_channels;
            }
            if (min_pixel <= max_attn_pixel) {
                encoder_blocks.insert(
                    "attn_" + std::to_string(i),
                    lifuren::nn::AttentionBlock(current_channels, num_heads, width * height / std::pow(2, 2 * i), num_groups).ptr()
                );
            }
            if (min_pixel > min_down_pixel) {
                encoder_blocks.insert(
                    "down_" + std::to_string(i),
                    lifuren::nn::Downsample(current_channels, num_groups).ptr()
                );
        ...
        // mixture
        {
            ...
            mixture_blocks.insert(
                "attn",
                lifuren::nn::AttentionBlock(current_channels, num_heads, width * height / std::pow(2, 2 * scales.size()), num_groups).ptr()
            );
            mixture_blocks.insert(
                "res_o",
                lifuren::nn::ResidualBlock(current_channels, current_channels, embedding_dims, num_groups).ptr()
        ...
        for (int i = scales.size() - 1; i >= 0; --i) {
            if (index >= num_skip_down) {
                decoder_blocks.insert(
                    ...
                    lifuren::nn::Upsample(current_channels, num_groups).ptr()
                );
                min_pixel *= 2;
            }
            for (int j = 0; j < num_res; ++j) {
        ...
        this->tail = this->register_module("tail", torch::nn::Sequential(
            ...
            torch::nn::SiLU(),
            torch::nn::Conv2d(torch::nn::Conv2dOptions(current_channels, out_channels, 3).padding(1).bias(false))
        ));
    }
    ~UNetImpl() {
